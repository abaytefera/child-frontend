    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 animate-in fade-in duration-700">
      ...
      <div className="lg:col-span-9 space-y-8">
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-500 font-medium">
              Welcome back, <span className="text-blue-600 font-bold">{user?.firstName}</span>. Here is the latest activity.
            </p>
          </div>
          <div className="flex gap-2">
            <button className="bg-white border border-slate-200 text-slate-600 px-4 py-2 rounded-xl text-sm font-bold hover:bg-slate-50 transition-all">Export Report</button>
            <button className="bg-blue-600 text-white px-4 py-2 rounded-xl text-sm font-bold shadow-lg shadow-blue-200 hover:bg-blue-700 transition-all">View All Childs</button>
          </div>
        </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            { label: "Total Students", val: childData?.[2]?.totalChild || 0, icon: faUsers, color: "blue" },
            { label: "Boys", val: childData?.[0]?.male || 0, icon: faMars, color: "indigo" },
            { label: "Girls", val: childData?.[1]?.female || 0, icon: faVenus, color: "rose" }
          ].map((stat, i) => (
            ...
            <div className="h-[220px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={trendData}>
                  ...
                  <Area type="monotone" dataKey="count" stroke="#3B82F6" strokeWidth={4} fill="url(#colorTrend)" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </div>
        ...
        <section className="bg-white rounded-[40px] border border-slate-100 shadow-sm overflow-hidden">
          <div className="p-8 border-b border-slate-50 flex justify-between items-center bg-slate-50/30">
            ...
            <Link to="/AllEmployees" className="text-blue-600 text-xs font-black uppercase tracking-widest hover:underline">View Directory</Link>
          </div>
          <div className="divide-y divide-slate-50 px-4">
            {employees?.slice(0, 4).map((emp) => (
              <div key={emp._id} className="flex items-center justify-between p-4 hover:bg-slate-50/80 rounded-2xl transition-all group">
                <div className="flex items-center gap-4">
                  <img src={emp.profile?.mediaurl || emp.profile} className="w-12 h-12 rounded-2xl object-cover ring-4 ring-white shadow-sm transition-transform group-hover:scale-110" alt="" />
                  <div>
                    <h4 className="font-bold text-slate-800 leading-none mb-1">{emp.firstName} {emp.lastName}</h4>
                    <span className="bg-blue-50 text-blue-600 text-[9px] font-black px-2 py-0.5 rounded-md uppercase">{emp.role}</span>
                  </div>
                </div>
                <Link to={`/EmployeeSingle/${emp._id}`} className="bg-white border border-slate-200 text-slate-600 px-5 py-2 rounded-xl text-xs font-bold hover:bg-blue-600 hover:text-white hover:border-blue-600 transition-all">
                  Profile
                </Link>
              </div>
            ))}
      ...
      <aside className="lg:col-span-3 space-y-6">
        ...
        {/* Support Card */}
        <div className="bg-gradient-to-br from-blue-600 to-indigo-700 p-8 rounded-[40px] text-white shadow-xl shadow-blue-500/20">
